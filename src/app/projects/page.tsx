...
export default function Page() {
	...
	const handleSave = async () => {
		...
		try {
			const saved = await pushProjects({
				projects,
				imageItems,
				expectedVersion: projectsDocument.version
			})

			setProjects(saved.data)
			setOriginalProjects(saved.data)
			...
			<div className='flex flex-col items-center justify-center px-6 pt-32 pb-12'>
				<div className='grid w-full max-w-[1200px] grid-cols-2 gap-6 max-md:grid-cols-1'>
					{projects.map((project, index) => (
						<ProjectCard key={project.name} project={project} isEditMode={isEditMode} onUpdate={handleUpdate} onDelete={() => handleDelete(project)} />
					))}
				</div>
			</div>
