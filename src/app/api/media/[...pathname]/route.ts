...
function parseResizeWidth(url: URL): number | null {
	const raw = url.searchParams.get('w')
	if (!raw) return null
	const width = Number.parseInt(raw, 10)
	if (!Number.isSafeInteger(width) || width < 16 || width > 3840) return null
	return width
}
...
export async function GET(request: Request, context: { params: Promise<{ pathname: string[] }> }): Promise<Response> {
	try {
		...
		const targetWidth = parseResizeWidth(url)
		...
		if (targetWidth && IMAGE_MIME_TYPES.has(contentType)) {
			...
			if (effectiveWidth < (metadata.width || 0)) {
				...
				return new Response(resized, {
					headers: {
						...
						ETag: `${result.blob.etag}-${effectiveWidth}`
					}
				})
			}
