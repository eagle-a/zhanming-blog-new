		...
		<div className='sticky flex w-[200px] shrink-0 flex-col items-start gap-4 self-start max-sm:hidden' style={{ top: 24 }}>
			...
			{stats && (
				<motion.div
					initial={{ opacity: 0, scale: 0.8 }}
					animate={{ opacity: 1, scale: 1 }}
					transition={{ delay: INIT_DELAY + ANIMATION_DELAY * 2.5 }}
					className='bg-card w-full rounded-xl border p-3 text-sm'>
					<h2 className='text-secondary mb-2 font-medium'>文章信息</h2>
					<div className='text-secondary space-y-1'>
						<div>字数：{stats.wordCount.toLocaleString()}</div>
						<div>预计阅读：{stats.readingTime} 分钟</div>
					</div>
				</motion.div>
			)}
