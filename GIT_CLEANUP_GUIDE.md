...
- 历史中曾出现的 GitHub App ID（非密码）
- 历史中曾出现的旧加密密钥（完整值不在当前文档中重复）
...
# 清理硬编码的 APP_ID
java -jar bfg.jar --replace-text --find-string "<OLD_APP_ID>" .

# 清理硬编码的 ENCRYPT_KEY
java -jar bfg.jar --replace-text --find-string "<OLD_ENCRYPT_KEY>" .
```
...
```bash
# 检查历史中是否还有敏感信息
git log -S "<OLD_APP_ID>" --source --all
git log -S "<OLD_ENCRYPT_KEY>" --source --all
...
def blob_callback(blob: Blob):
    if blob.data:
        # 替换 APP_ID（填入实际旧值后执行）
        blob.data = blob.data.replace(b'<OLD_APP_ID>', b'REMOVED_APP_ID')
        # 替换 ENCRYPT_KEY（填入实际旧值后执行）
        blob.data = blob.data.replace(b'<OLD_ENCRYPT_KEY>', b'REMOVED_ENCRYPT_KEY')
