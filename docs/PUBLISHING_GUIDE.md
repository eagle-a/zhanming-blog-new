...
`date` 就是发布时间，而且**写未来时间等于定时发布**：批准之后文章虽然已经是 `published`，但可见性条件要求 `published_at <= now()`，所以到点之前访客打开文章链接只会拿到 404，连 CDN 也会把这个 404 缓存几分钟。日常投稿写当前时间，或者干脆不写 `date` 用文件修改时间；只有确实要排期才写未来时间。CLI 遇到未来时间会在取投稿码之前先警告，`/admin/review` 的编辑区和批准确认框也会标出「定时发布」。
...
注意例外：如果上一条的 `date` 是未来时间，批准不会让它立刻可见，见 2.1 的说明。
