...
1. 新建文章打开 `https://zhanmingblog.cc.cd/write`；编辑已有文章打开 `/write/<slug>`，例如 `/write/weekly-report-2026-09-23`。两个入口是不同路由，别拿新 slug 拼地址当新建用。
...
编辑已有文章进 `/write/<slug>`，**编辑模式不允许改 slug**（新建时 slug 自己填）。如果两个会话同时改同一篇，服务端返回 `409`，刷新页面重做这次修改，不要强行覆盖。
...
`date` 就是发布时间，而且**写未来时间等于定时发布**：批准之后文章虽然已经是 `published`，但可见性条件要求 `published_at <= now()`，所以到点之前访客打开文章链接只会拿到 404，连 CDN 也会把这个 404 缓存几分钟。

所以 CLI **默认拒绝未来时间**，在取投稿码之前就停下（不浪费一次性投稿码），并告诉你把 `date` 删掉或改成当前时间。确实要排期才加 `--schedule`：

```powershell
pnpm --dir C:\Users\zm\Desktop\project\zhanming-blog-new agent:submit C:\path\to\article.md -- --schedule
```

不要凭手写时间猜：日常投稿写当前时间，或者干脆不写 `date` 用文件修改时间。
...
注意例外：如果上一条的 `date` 是未来时间，批准不会让它立刻可见，见 2.1 的说明。这种情况下审批台的编辑区会标出「定时发布」并给出倒计时，**发布时间那栏可以直接改**（改成当前时间就立即公开），不必打回重投；批准确认框会再提醒一次。
