...
> 2026-09-14 修订：审稿草稿增加本地修改保护，保存/批准/拒绝绑定内容哈希；列表采用游标分页摘要，正文按需读取。新增隔离浏览器测试和进程内 PostgreSQL 仓储测试。生产发布入口为 `pnpm deploy:production`，验证实时远端 SHA 和 GitHub CI。自动代理命名管道规则与 HTTP CLI 不兼容，当前代理只能准备 Markdown，参见投稿指南；下文历史“AI 投稿可用”不代表代理获准调用 HTTP。
