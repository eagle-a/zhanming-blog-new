...
2026-09-24 起，**内容图片（文章正文、封面、站点配图、博客/分享/图集列表）只在 Blob 里有一份**，仓库不再保留副本：`public/images/` 只剩站点外壳素材（`avatar.png`、`cursor.svg`、`christmas/`、`hats/`、`share/tinypng.png`）。直接后果是：没有 `DATABASE_URL` 的回退渲染仍然可用，但那些页面上的图片需要 Blob 可达，离线自包含已经不是目标。

## 2. 图片的两条通道

新周报的图片走 Blob：先上传（管理后台上传，或先用第 5 节命令从 `public/images/<slug>/` 临时中转），正文引用返回的 `/api/media/...` 路径，**确认线上 200 之后删掉本地中转副本再提交**。站点外壳素材仍然走仓库 `public/`，由 `/_next/image` 优化。
